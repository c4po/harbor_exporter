...
func (e *Exporter) collectReplicationsMetric(ch chan<- prometheus.Metric, pageSize int) bool {
	...
	result := true
	for {
		policiesBody := e.client.request(fmt.Sprintf("/replication/policies?page=%d&page_size=%d", page, pageSize))
		page++
