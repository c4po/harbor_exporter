...
func (e *Exporter) collectReplicationsMetric(ch chan<- prometheus.Metric) bool {
	...
	policiesBody := e.client.request("/api/v2.0/replication/policies")
	...
	for i := range policiesData {
		...
		body := e.client.request("/api/v2.0/replication/executions?policy_id=" + policyId + "&page=1&page_size=1")
